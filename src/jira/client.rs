...
impl JiraClient {
    /// Creates a new Jira client with the given credentials
    ...
    /// Creates authorization headers for Jira API requests
    fn create_headers(&self) -> Result<HeaderMap, Error> {
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&auth_header).map_err(|e| {
                Error::JiraApiError(format!("Failed to create auth header: {}", e))
            })?,
        );
    ...
    /// Updates the checklist field of a Jira issue
